...
//	DMA->channel[0].CCR    &= ~0x7FFF;
//	DMA->channel[0].CNDTR  &= ~0xFFFF;

	DMA->channel[channel_number].CCR |= 0x00005AC2;
/*
// Setting number of data
	DMA->channel[0].CNDTR = 1000;
	// Setting Peripheral address
	DMA->channel[0].CPAR = (u32)arr1;
	// Setting memory address
	DMA->channel[0].CMAR = (u32)arr2;
	*/
	// NVIC setup
	...
	// Enable DMA channel
//	DMA->channel[channel_number].CCR |= 0x00000001;
}
